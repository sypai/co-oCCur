...
int co_oCCur::AlignFP::findMax(std::vector<int> InVector) {
    if (!InVector.empty()) {
        return *std::max_element(InVector.begin(), InVector.end());;
    }
    return INT32_MIN;
...
std::vector<uint32_t > co_oCCur::AlignFP::sliceIt0(const std::vector<uint32_t > &InArray, int lb, int ub)
{
...
std::vector<double> co_oCCur::AlignFP::sliceIt(const std::vector<double> &InArray, int lb, int ub)
{
...
double co_oCCur::AlignFP::roundIt(double var)
{
...
void co_oCCur::AlignFP::brum_brum()
{
    ...
    edit->Adjust(m_OriginalSubtitleFile, m_Segments);
}
